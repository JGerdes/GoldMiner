...
Gameoverscreen::Gameoverscreen(SpriteManager* spriteManager, bool enable) :
		...
		text_(""),
		score_(0){
		Button *b = new Button(playAgain, spriteManager->getSprite("assets/graphics/button.ppm"),Vec2((1280-64*6)/2.0,200), Vec2(64*6,16*8));
		b->setFont(font_);
		b->setText("Play again!");
		buttons_.push_back(b);

		b = new Button(mainMenu, spriteManager->getSprite("assets/graphics/button.ppm"),Vec2(1280-32*6-20,100), Vec2(32*6,8*6));
		b->setFont(font_);
		b->setText("Main menu");
		buttons_.push_back(b);
}
...
void Gameoverscreen::draw()const{
	...
	font_->setSize(4);
	font_->draw_text(Vec2((1280-font_->compute_dimension(text_).getX())/2, 400), text_);
	stringstream scoreText;
	scoreText << "Score: " << score_;
	font_->setSize(2);
	font_->draw_text(Vec2((1280-font_->compute_dimension(scoreText.str()).getX())/2, 350), scoreText.str());
}
