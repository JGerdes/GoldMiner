...
Highscorescreen::Highscorescreen(SpriteManager* spriteManager, bool enable):
	...
	Button* b = new Button(Highscorescreen::backButton,  spriteManager->getSprite("assets/graphics/button.ppm"),Vec2(1280-32*6-20,100), Vec2(32*6,8*6));
	b->setFont(font_);
	b->setText("back");
...
void Highscorescreen::draw()const{
	...
	font_->draw_text(Vec2((1280-font_->compute_dimension(title).getX())/2,460),title);
	font_->setSize(3);
	font_->draw_text(Vec2(300 ,400),lvl1);
	font_->draw_text(Vec2(1280-300-font_->compute_dimension(lvl2).getX() ,400),lvl2);
	...
	for(string score : level_2_){
		font_->draw_text(Vec2(1280-300-font_->compute_dimension(score).getX(), 350-40*i), score);
		++i;
	}
